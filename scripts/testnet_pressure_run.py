...
def main():
    ...
    parser.add_argument("--existing-share-coin-id", default="")
    ...
    if args.reuse_existing_state:
        if not args.existing_share_coin_id:
            raise SystemExit("--existing-share-coin-id is required when --reuse-existing-state is used")
        withdraw = cycle_probe.request_queued_withdraw(manifest, args.existing_share_coin_id, args.clock_id)
        withdraw_steps.append(withdraw)
        report["steps"].append({"step": "request_withdraw_entry_reused_share_coin", **withdraw})
    else:
        deposits = []
        ...
        for index, deposit in enumerate(deposits):
            withdraw = cycle_probe.request_queued_withdraw(manifest, deposit["share_coin_id"], args.clock_id)
            withdraw_steps.append(withdraw)
